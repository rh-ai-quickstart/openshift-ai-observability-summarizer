...
def get_mcp_server_executable(project_root: Path) -> str:
    """Get the correct MCP server executable path."""
...
def generate_claude_config(mcp_server_path: str) -> Dict[str, Any]:
...
def generate_cursor_config(mcp_server_path: str) -> Dict[str, Any]:
...
def test_mcp_server(mcp_server_path: str) -> bool:
    """Test if the MCP server executable works."""
    try:
        ...
        # Try to run with --help to test basic functionality
        result = subprocess.run(
            [mcp_server_path, "--help"],
            capture_output=True,
            text=True,
            timeout=10
        )
...
def main():
    ...
    # 2. Get MCP server executable path
    mcp_server_path = get_mcp_server_executable(project_root)
    print(f"🔧 MCP server: {mcp_server_path}")
    
    # 3. Test MCP server
    if not test_mcp_server(mcp_server_path):
    ...
    claude_config = generate_claude_config(mcp_server_path)
    ...
    cursor_config = generate_cursor_config(mcp_server_path)
